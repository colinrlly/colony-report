...
export default function Home() {
  ...
  // Window stacking order - windows at the end of the array are on top
  const [windowStack, setWindowStack] = useState<string[]>([]);

  // Bring a window to front by moving it to the end of the stack
  ...
  const getWindowZIndex = useCallback((windowId: string) => {
    ...
    // Base z-index of 20, increment for each position in stack
      ...
      <main ref={mainRef} className="min-h-screen relative p-4 pt-[46px] pb-[50px]">
        ...
        {openCameras[3] && !minimizedCameras[3] && (
          <SecurityCameraFeed
            ...
            zIndex={getWindowZIndex('cam-3')}
            onFocus={() => bringToFront('cam-3')}
          />
        )}
